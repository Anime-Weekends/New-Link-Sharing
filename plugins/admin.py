...
waiting_for_admin_input = {}  # Stores states for adding admins
...
async def add_admin_cb(client, query: CallbackQuery):
    ...
    waiting_for_admin_input[query.message.chat.id] = query  # Store query
...
async def handle_add_admin_input(client, message: Message):
    ...
    await message.delete()  # Delete UID message
    ...
    status = (
        f"<pre>✔ Usᴇʀ <code>{user_id}</code> ᴀᴅᴅᴇᴅ ᴀs ᴀᴅᴍɪɴ</pre>"
...
async def deladmin_cb(client, query: CallbackQuery):
    ...
    status = (
        f"<pre>✔ Rᴇᴍᴏᴠᴇᴅ <code>{user_id}</code> ғʀᴏᴍ ᴀᴅᴍɪɴs</pre>"
        if success else f"<pre>✘ Fᴀɪʟᴇᴅ ᴛᴏ ʀᴇᴍᴏᴠᴇ <code>{user_id}</code></pre>"
    )
